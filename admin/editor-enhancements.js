(function(){
  ...
  function enhance(){
    ...
    var flow=D.createElement("div");flow.className="ah-editor-flow";flow.innerHTML=[t("1 · Данни","1 · Details"),t("2 · Снимки","2 · Photos"),"3 · BG + EN",t("4 · Проверка","4 · Review"),t("5 · Публикуване","5 · Publish")].map(function(s){return "<span>"+s+"</span>";}).join("");
    ...
    quick.querySelector("input").placeholder=t("AutoHaus URL или slug","AutoHaus URL or slug");quick.querySelector("input").setAttribute("aria-label",quick.querySelector("input").placeholder);quick.querySelector("button").textContent=t("Импортирай от AutoHaus","Import from AutoHaus");
    var source=D.getElementById("source-text");
    if(source){var lab=source.closest("label");if(lab){var span=lab.querySelector("span");if(span){span.className="ah-source-label";span.textContent=t("Оригинален текст · за AI обработка","Source text · for AI processing");}var note=D.createElement("div");note.className="ah-source-note";note.textContent=t("Поставете текста, обработете и прегледайте BG + EN.","Paste, process, then review BG + EN.");lab.insertAdjacentElement("afterend",note);}}
    ...
    if(btn)btn.onclick=async function(){if(!window.AH_ADMIN.canLeave())return;var value=slug(input.value);if(!/^[a-z0-9-]{1,180}$/.test(value)){alert("Постави валиден AutoHaus URL или slug.");input.focus();return;}btn.disabled=true;btn.textContent=t("Импортиране…","Importing…");try{var r=await fetch("/api/admin/sync?action=vehicle",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({slug:value,sort_order:0})});var d=await r.json().catch(function(){return{};});if(!r.ok)throw new Error(d.error||"Импортът не успя.");window.AH_ADMIN.reloadVehicle(d.id);}catch(e){alert(e.message);btn.disabled=false;btn.textContent=t("Импортирай от AutoHaus","Import from AutoHaus");}};
  }
  window.AH_ADMIN.enhanceEditor=enhance; enhance();
})();
