...
(function () {
  ...
  /* ---- SEARCH -------------------------------------------------------------
     ...
     · The dataset is written in Latin for the make and Cyrillic for
       everything else, but Bulgarian buyers type makes both ways. Each
  ...
  if (wall) {
    ...
    var sync = function () {
      ...
      /* the thumb WIDTH only moves on a resize, not on a scroll — writing it
         every scrolled frame dirtied layout for nothing. Its POSITION (the
         transform below) is the part that tracks the finger, and that stays
         on the compositor. */
      if (w !== lastW) { bar.style.width = w + "%"; lastW = w; }
    ...
    AH.openFocus = function (id, opener) {
      ...
      fBody.innerHTML = '' +
        ...
        '<dl class="focus-spec body-s">' +
          ...
          "<div><dt>Гориво</dt><dd>" + FUEL[v.fuel] + "</dd></div>" +
